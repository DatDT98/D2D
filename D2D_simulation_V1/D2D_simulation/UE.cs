...
namespace D2D_simulation
{
    class UE
    {
        ...
        public double PassLoss_Indoor(Position pos)
        {
            double passloss;
            double distance = this.Position.distance(this.Position, pos);

            passloss = 127 + 30 * Math.Log10(distance * Math.Pow(10, -3));
            return passloss;
        }
        public double PassLoss_Outdoor(Position pos)
        {
            double passloss;
            double distance = this.Position.distance(this.Position, pos);

            passloss = 128.1 + 37.6 * Math.Log10(distance * Math.Pow(10, -3));
